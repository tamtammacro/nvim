...
entry.get_word = function(self)
	return self.cache:ensure("get_word", function()
		...
		local word
		if
			misc.safe(self:get_completion_item().textEdit)
		...
		if misc.safe(self:get_completion_item().textEdit) then
			...
			if range then
				return self.context.cache:ensure(
					{ "entry", "get_overwrite", tostring(range.start.character), tostring(range["end"].character) },
					function()
...
entry.is_deprecated = function(self)
	return self:get_completion_item().deprecated
		or vim.tbl_contains(self:get_completion_item().tags or {}, types.lsp.CompletionItemTag.Deprecated)
end
...
entry.convert_position_encoding = function(self, position)
	...
	return self.context.cache:ensure(
		"entry.convert_position_encoding." .. position.character .. "." .. from_encoding,
		function()
